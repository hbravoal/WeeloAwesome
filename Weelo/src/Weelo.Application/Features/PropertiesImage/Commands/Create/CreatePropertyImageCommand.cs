...
namespace Weelo.Application.Features.PropertiesImage.Commands.Create
{
    ...
    public class CreatePropertyImageCommandHandler : IRequestHandler<CreatePropertyImageCommand, Response<int>>
    {
        ...
        public CreatePropertyImageCommandHandler(IPropertyImageRepositoryAsync repository, IMapper mapper, Microsoft.Extensions.Hosting.IHostEnvironment environment)
        {
        ...
        public async Task<Response<int>> Handle(CreatePropertyImageCommand request, CancellationToken cancellationToken)
        {
            ...
            //TODO: Optimizing GetEntryAssembly -> Why? Visual's Daemon don't work for autoc for usings.
                try
                {
                string path = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location), "uplods");
                if (!System.IO.Directory.Exists(path))
                    {
                        System.IO.Directory.CreateDirectory(path);
                    }

                string op = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location)+ "\\uploads\\" + request.File.FileName.ToString();
                

                using (System.IO.FileStream filestream = System.IO.File.Create(path+request.File.FileName))
                    {
                        request.File.CopyTo(filestream);
                        filestream.Flush();
                        string filepath= "\\uploads\\" + request.File.FileName;
